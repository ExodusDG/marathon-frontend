...
function AddGroup(props) {
    ...
    function addGroup() {
        if (name.length > 0 && login.length > 0 && selectedShelude !== 'Select...' && date.length > 0) {
            ...
            var config = {
                method: 'post',
                url: process.env.REACT_APP_API_URL + '/editGroup',
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                },
                data: group
    ...
    useEffect(() => {
        ...
        setName(props.data.group_name)
        setLogin(props.data.group_id)
        setSelectedShelude(props.data.shelude)
        onChange(new Date(moment(props.data.group_start_date).format('MM-DD-YYYY')))
    }, [props.data])


    return (
        <div className="userPopup addGroupPopup">
            <div className="userPopupBlock">
                <div className="popupClose">
                    <img src={close} alt="Close" onClick={e => props.setEditGroupPopup(false)} />
                </div>
                <p className="userPopupTitle">Edit Group</p>
                ...
                <button onClick={e => addGroup()}>Save changes</button>
            </div>
        </div>
    );
}
